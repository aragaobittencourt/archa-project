...
function Calculator() {
  ...
  // Initialize Session ID once (Per Session/Tab)
  useEffect(() => {
    let storedSession = sessionStorage.getItem('archa_session_id');
    if (!storedSession) {
      storedSession = uuidv4();
      sessionStorage.setItem('archa_session_id', storedSession);
    }
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      // GUARD CLAUSE: Don't save "Ghost" rows (empty users)
      // Only save if the user has actually started typing their contact info
